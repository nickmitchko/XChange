...
import info.bitrich.xchangestream.kraken.dto.KrakenSocketOHLC;
...
public class KrakenStreamingMarketDataService implements StreamingMarketDataService {
  ...
  public Observable<KrakenOHLC> getOHLC(
      CurrencyPair currencyPair, Integer interval, Object... args) {
    ...
    return subscribeRaw(channelName, 1, null, interval)
        ...
        .map(
            OHLCList -> {
              ...
              double time = getValue(OHLCList, 0, double.class);
              double etime = getValue(OHLCList, 1, double.class);
              ...
              long currentTime =
                  (long) time; // Will automatically cut off the extra epoch precision
              long endTime = (long) etime;
              return new KrakenSocketOHLC(
                  currentTime,
                  endTime,
